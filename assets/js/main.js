...
document.addEventListener('click', function (e) {
    ...
    if (link && link.href.includes('go.fiverr.com')) {
        ...
        if (typeof gtag === 'function') {
            // Prevent default only if opening in same tab to wait for callback
            if (link.target !== '_blank') {
                e.preventDefault();

                // Safety timeout: if Google doesn't respond in 1s, go anyway
            ...
            // 2. Send New Conversion ID
            gtag('event', 'conversion', {
                'send_to': 'AW-17297709178/P9gECI6p6dMbEPqwmLhA',
                'event_callback': link.target !== '_blank' ? handleNavigation : undefined
            });

            console.log('Conversion events sent to Google Ads (Dual IDs)');
        }
    }
});
